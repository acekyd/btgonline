import { updateTeam } from '../../../services/admin/team.admin.service'

export default defineEventHandler(async (event) => {
  const id = parseInt(getRouterParam(event, 'id') ?? '', 10)
  if (isNaN(id)) {
    throw createError({ statusCode: 400, message: 'Invalid id' })
  }

  const body = await readBody(event)
  if (!body || Object.keys(body).length === 0) {
    throw createError({ statusCode: 400, message: 'Request body is empty' })
  }

  const data: { canonical_name?: string; logo_url?: string } = {}
  if (body.canonical_name !== undefined) data.canonical_name = String(body.canonical_name).trim()
  if (body.logo_url !== undefined) data.logo_url = String(body.logo_url).trim()

  const updated = updateTeam(id, data)
  if (!updated) {
    throw createError({ statusCode: 404, message: 'Team not found' })
  }

  return { data: updated }
})
